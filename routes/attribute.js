...
router.get('/AttributesData' , function(req, res, next) {
  ...
  router.get('/Attribute_AddAttribute', function(req, res, next) {
    if (req.session.loggedin) {
        ...
        con.query(sql, function (err, data, fields) {
         ...
         res.render('Attribute_AddAttribute',{message : req.flash('message'),Sidebar:data,field:field,parent:data2});
  ...
  router.post('/addattribute', (req,res)=>{
    ...
    const values =[[att_name,att_type,att_position,att_number,isparent,key_name]]
  ...
  router.get('/Attribute_ListAttribute', function(req, res, next) {
    ...
    con.query(sql, function (err, data, fields) {
     if (err) throw err; 
     var sql1='SELECT * FROM attribute';
     con.query(sql1, function (err1, data1, fields1) {
      if (err1) throw err1;
      
      var field =['Number','Text','Text Area','Search Radio','Searchable Checkbox','Searchable Dropdown','Color'];
     res.render('Attribute_ListAttribute',{ListData: data1,message : req.flash('message'),Sidebar:data,field:field});
